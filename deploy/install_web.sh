...
function usage(){
	echo "Usage: $0 -m centos/ubuntu(will install mysql first) -i(will initial database, including create database and tables)"
}
...
##Check if need install mysql first
if [[ $install_mysql = "true" ]]; then
	echo "installing mysql..."
	sh $SCRIPT_DIR/install_mysql.sh $MYSQL_USR $os $MYSQL_PASS
fi
...
if [[ $initial_db = "true" ]]; then
	...
	if [[ $hadoop_node_count -eq 0 ]]; then
		...
		index=0
		echo "INSERT INTO \`hadoopnodes\` (\`hostname\`,\`ip\`,\`os\`,\`sshport\`,\`sshuser\`,\`sshpass\`,\`issudo\`,\`namenode\`,\`datanode\`,\`secondarynamenode\`,\`resourcemanager\`,\`nodemanager\`,\`isformatted\`)  VALUE" >> $SCRIPT_DIR/autosys.sql
	...
	if [[ $spark_node_count -eq 0 ]]; then
		...
		index=0
		echo "INSERT INTO \`sparknodes\` (\`hostname\`,\`ip\`,\`os\`,\`sshport\`,\`sshuser\`,\`sshpass\`,\`master\`,\`slave\`,\`issudo\`)  VALUE" >> $SCRIPT_DIR/autosys.sql
